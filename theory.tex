...
\begin{minipage}[t]{0.47\textwidth}
\begin{statementblock}
\Psi(takeKey, \sigma_0) = \sigma_1 \byrule{(M1)} \\
\Psi(takeKey, \sigma_1) = \sigma_1 \byrule{(M1)} \\
\Psi(takeKey, \sigma_2) = \sigma_3 \byrule{(M1)} \\
\Psi(takeKey, \sigma_3) = \sigma_3 \byrule{(M1)}
\end{statementblock}
\end{minipage}
\hfill
\begin{minipage}[t]{0.47\textwidth}
...
\caption{Whole state-transition graph for Example 2. The highlighted vertex marks the initial state, and the graph shows the three ways in which cost 0 can arise.}
\end{figure}
...
\begin{minipage}[t]{0.47\textwidth}
\begin{statementblock}
\Psi(takeKey, \sigma_0) = \sigma_0 \byrule{(M1)} \\
\Psi(takeKey, \sigma_1) = \sigma_1 \byrule{(M1)} \\
\Psi(takeKey, \sigma_2) = \sigma_0 \byrule{(M1)} \\
\Psi(takeKey, \sigma_3) = \sigma_1 \byrule{(M1)}
\end{statementblock}
\end{minipage}
\hfill
\begin{minipage}[t]{0.47\textwidth}
...
\caption{Whole state-transition graph for Example 3 in Model 1.}
\end{figure}
...
\caption{Whole state-transition graph for Example 3 in Model 2.}
\end{figure}
...
\caption{Whole state-transition graph for Example 4. The highlighted vertex marks the only initial state that survives the value statement.}
\end{figure}
